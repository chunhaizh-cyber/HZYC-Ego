...
private:
    ...
    static std::string 获取新主键(std::string& 当前主键) {
        if (当前主键.empty()) return "0";
        std::string 结果 = 当前主键;
...
//    }
    ...
    // —— 修改节点 —— 
    void 修改节点信息(std::string& 主键, 主信息类型& 新主信息) {
        std::lock_guard<std::mutex> lock(链表锁);
        节点类* 节点 = 查找主键(主键);
        if (!节点) return;
