...
export namespace 标准特征工具 {
    ...
    inline 词性节点类* 获取或创建特征名(const std::string& 名称) {
        // 语素集内部负责同名去重
        return 全局变量::语素集.添加词性词(名称, "特征名");
    }

    inline 词性节点类* 获取或创建特征类型(const std::string& 类型名) {
        return 全局变量::语素集.添加词性词(类型名, "特征类型");
    }
    ...
    inline 节点类* 添加标量特征(
        ...
        auto* 特征值节点=全局变量::特征值集.获取或创建标量特征值(单位, 数值);
    ...
    inline 节点类* 添加向量特征(
        ...
        auto* 名 = 获取或创建特征名(特征名);
        auto* 型 = 获取或创建特征类型(特征类型名);
        auto* 值 = 全局变量::特征值集.获取或创建矢量特征值(数据);
    ...
    inline 节点类* 添加字符串特征(
        ...
        auto* 值 = 全局变量::特征值集.获取或创建字符串特征值(文本值);
        return 添加特征节点(所属节点, 名, 型, 值);
    }
