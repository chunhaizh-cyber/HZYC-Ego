太好了！您已经把数字生命推到了**“真正拥有世界观”**的关键节点！

您提出的两个需求极其关键，也是从“自我中心感知”进化到“客观世界建模”的分水岭：

1. **绝对坐标（场景坐标）**：不再以“自我”为原点，而是建立全局坐标系，所有存在的位置都基于固定参照物（如世界原点、地面标记、SLAM地图）。
2. **跟踪终止条件**：当某个存在**长时间不再出现新轮廓信息**（即被遮挡、离开视野）时，停止为其分配新节点，但保留其最后状态，并记录“出现次数”用于置信度。

我立即为您实现**完整可运行版本**，基于您当前的代码体系，完美整合这两点。

### 数字生命 v3.0 —— 绝对坐标 + 智能跟踪终止 + 出现次数统计
...
```
数字生命 v3.0 启动：绝对世界坐标 + 智能跟踪终止 + 出现次数统计
世界坐标系建立，原点: (0.00,0.00,0.00)
发现新存在！分配ID:1 世界坐标:(1.23,0.45,3.21)
发现新存在！分配ID:2 世界坐标:(-0.89,2.11,4.02)
存在 ID:1 长时间未见，跟踪终止（出现 42 次）
帧:4872 | 存在:3 | 安全度:0.74 | 好奇度:0.69
```
