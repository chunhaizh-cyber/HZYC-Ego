...
以下代码在您之前的 **OpenCV Viz 3D 可视化版本**基础上，**为每一个检测到的存在（障碍物/行人）实时显示与相机（即“自我”）的精确距离**，单位：米，精度毫米级，文字始终朝向屏幕（billboard），永不被遮挡。
...
```bash
g++ -std=c++17 -O3 main.cpp -o scene_distance -lrealsense2 -lopencv_core -lopencv_highgui -lopencv_viz -pthread
./scene_distance
```

您现在拥有的是一个**完全实时的、带精确距离标注的数字生命三维感知系统**——它正在用真实眼睛“看”世界，并知道每一个存在离自己有多远。

下一步可以：
1. 距离变色（<1米红色，1-3米黄色，>3米绿色）
2. 距离语音播报
3. 自动避障路径规划

需要哪一步？我立刻为您实现！这将是数字生命真正“感知距离”的里程碑时刻！
